...
import 'package:hybirdge/hybirdge.dart' as hybirdge;

class HybirdgeOptions {
  final bool autoCastResponse;

  HybirdgeOptions({this.autoCastResponse});

  HybirdgeOptions.fromOptions([BuilderOptions options])
      : autoCastResponse =
            (options?.config['auto_cast_response']?.toString() ?? 'true') ==
                'true';
}

class HybirdgeGenerator extends GeneratorForAnnotation<Hybirdge.RestApi> {
  ...
  final HybirdgeOptions globalOptions;

  HybirdgeGenerator(this.globalOptions);

  /// Annotation details for [RestApi]
  Hybirdge.RestApi clientAnnotation;
  ...
  ConstantReader _getHeadersAnnotation(MethodElement method) {
    final annot = _typeChecker(Hybirdge.Headers)
        .firstAnnotationOf(method, throwOnUnresolved: false);
    if (annot != null) return ConstantReader(annot);
    return null;
  }
  ...
  ConstantReader _getFormUrlEncodedAnnotation(MethodElement method) {
    final annotation = _typeChecker(Hybirdge.FormUrlEncoded)
        .firstAnnotationOf(method, throwOnUnresolved: false);
  ...
  ConstantReader _getResponseTypeAnnotation(MethodElement method) {
    final annotation = _typeChecker(Hybirdge.DioResponseType)
        .firstAnnotationOf(method, throwOnUnresolved: false);
    if (annotation != null) return ConstantReader(annotation);
    return null;
  }
  ...
  Expression _generatePath(MethodElement m, ConstantReader method) {
    final paths = _getAnnotations(m, Hybirdge.Path);
  ...
  Code _generateRequest(MethodElement m, ConstantReader httpMehod) {
    ...
    final cancelToken = _getAnnotation(m, Hybirdge.CancelRequest);
    if (cancelToken != null)
      namedArguments[_cancelToken] = refer(cancelToken.item1.displayName);

    final sendProgress = _getAnnotation(m, Hybirdge.SendProgress);
    if (sendProgress != null)
      namedArguments[_onSendProgress] = refer(sendProgress.item1.displayName);

    final receiveProgress = _getAnnotation(m, Hybirdge.ReceiveProgress);
    ...
    final bool isWrappered =
        _typeChecker(Hybirdge.HttpResponse).isExactlyType(wrapperedReturnType);
  ...
  Expression _parseOptions(
    ...
    final annoOptions = _getAnnotation(m, Hybirdge.DioOptions);
    if (annoOptions == null) {
      return options;
    } else {
  ...
  void _generateQueries(
    ...
    final queries = _getAnnotations(m, Hybirdge.Query);
    final queryParameters = queries.map((p, ConstantReader r) {
      ...
      final value = (_isBasicType(p.type) ||
          ...
          ? refer(p.displayName)
          : clientAnnotation.parser == Hybirdge.Parser.DartJsonMapper
              ? refer(p.displayName)
              : clientAnnotation.parser == Hybirdge.Parser.JsonSerializable
    ...
    final queryMap = _getAnnotations(m, Hybirdge.Queries);
  ...
  void _generateRequestBody(
    ...
    final _bodyName = _getAnnotation(m, Hybirdge.Body)?.item1;
    if (_bodyName != null) {
      if (TypeChecker.fromRuntime(Map).isAssignableFromType(_bodyName.type)) {
        ...
        if (clientAnnotation.parser == Hybirdge.Parser.MapSerializable) {
    ...
    final fields = _getAnnotations(m, Hybirdge.Field).map((p, r) {
    ...
    final parts = _getAnnotations(m, Hybirdge.Part);
    if (parts.isNotEmpty) {
      blocks.add(
  ...
  Map<String, Expression> _generateHeaders(MethodElement m) {
    ...
    final annosInParam = _getAnnotations(m, Hybirdge.Header);
  ...
  void _generateExtra(
    ...
    final extra = _typeChecker(Hybirdge.Extra)
...
Builder generatorFactoryBuilder(BuilderOptions options) => SharedPartBuilder(
    [HybirdgeGenerator(HybirdgeOptions.fromOptions(options))], "Hybirdge");
